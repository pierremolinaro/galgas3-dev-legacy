...
Un commentaire commence par les caractères « \tpp{\//} ») et s'étend jusqu'à la fin de la ligne courante.
...
Sous Windows, si aucun fichier source à compiler n'est indiqué dans la ligne de commande, GALGAS affiche un dialogue proposant d'entrer ce fichier. L'option \tpp{-{}-no-dialog} (\refSectionPage{optionGenerales}) permet de ne pas faire apparaître ce dialogue.
...
\begin{enumerate}
  ...
  \item en GALGAS, un nom de type est un identificateur précédé du caractère \tpp{@}~; par exemple~: \ggst+@string+, \ggst+@uint+, \ggst+@lstring+, \ggst+@luint+, …~;
...
La règle \ggst+list delimitorsList style delimitersStyle error ...+ déclare les terminaux \ggst+$:$+, \ggst+$,$+… Les messages d'erreur syntaxique sont définis en remplaçant la séquence \tpp{\%K} par l’épellation du délimiteur.
...
\begin{enumerate}
  ...
  \item \ggst+syntax logo_syntax+~: les règles de productions sont dans le composant syntaxique \ggst+logo_syntax+, situé dans le fichier \tpp{logo-syntax.galgas}~;
...
Par défaut dans le fichier \tpp{logo\_syntax.galgas}, seul le non terminal \ggst+<start_symbol>+ est déclaré, et une règle de production vide est écrite.
...
Si des erreurs d'analyse de la grammaire surviennent, vous pouvez ajouter dans la ligne de commande l'option \tpp{-{}-output-html-grammar}~: celle-ci provoquera la génération du fichier \tpp{chezmoi/logo/build/helpers/lo}\-\tpp{go\_grammar.html} qui contient tous les détails de l'analyse de la grammaire.
...
Dans l’analyseur syntaxique, pour chaque occurrence du token \ggst+$identifier$+, nous avons précédemment écrit \ggst+$identifier$ ?*+ pour signifier que la valeur de la chaîne de caractères n’était pas utilisée.
...
Pour prendre en compte l’instruction \tpp{CALL}, nous allons procéder comme suit~: d’abord, la définition du type table \ggst+@routineMap+ va être modifier de façon à associer à chaque routine la liste mise à plat des instructions. Ensuite, nous prendrons en compte l’instruction \tpp{CALL} en extrayant de la table des routines la liste des instructions de la routine appelée, et en l’insérant à la fin de la liste courante des instructions.
...
{\bf Prise en compte de l'instruction \tpp{CALL}.} Il suffit d'ajouter à la liste courante des instructions toutes les instructions de la routine appelée par \tpp{CALL}~:
\begin{galgas3}
...
...
L'instruction \ggst+for+ permet d’énumérer un objet de type liste. Le corps de la boucle (entre \ggst+do+ et \ggst+end+) est exécuté une fois pour chaque élément \ggst+i+ de la liste.
...
L'allure du calcul des tracés est la suivante (à placer à la fin de la règle \ggst+<start_symbol>+) dans \tpp{logo-syntax.galgas}~:
...
Elle est nommée par exemple \ggst+codeDisplay+ et on peut la déclarer dans n'importe que fichier~; par souci de simplicité, on choisit le fichier qui contient les déclarations sémantiques, c'est-à-dire \tpp{chezmoi/logo/galgas-sources/logo-semantics.galgas}.
...
Par exemple, pour la classe \ggst+@penUp+, la surcharge de la méthode \ggst+codeDisplay+ est la suivante. Pour la même raison que précédemment, on place cette déclaration dans le fichier de définitions sémantiques \tpp{chezmoi/logo/galgas-sources/logo-semantics.galgas}.
...
Le calcul des tracés dans \tpp{logo-syntax.galgas} peut être complété par l'appel de la méhode \ggst+codeDisplay+ pour chaque instruction.
\begin{galgas3}
  ...
  @bool pendown = false
  @double x = 0.0
...
Notez~:
\begin{itemize}
  \item l'échappement des caractères \tpp{\%}~: pour obtenir \texttt{"100\%"}, on écrit \tpp{"100\textbackslash\%"}~;
...
La liste des instructions de tracé est accumulée dans une chaîne de caractères. Modifier toutes les méthodes \ggst+codeDisplay+ de façon à construire cette chaîne en ajoutant un argument formel en entrée/sortie~: \tpp{?!@string SVG}. Il faut modifier la méthode \ggst+codeDisplay+ de la classe \ggst+@forward+ pour ajouter la génération de code~:
